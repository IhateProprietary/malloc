...
# define SIZE_IS_MMAPPED	0x2
# define SIZE_FLAGS			(SIZE_PREV_INUSE | SIZE_IS_MMAPPED)
...
# define DEFAULT_MMAP_SIZE		65536
# define DEFAULT_MMAP_THRESHOLD	(128 * 1024)
# define DEFAULT_TRIM_SIZE		(128 * 1024)
...
# define ISREQ_IN_SMALLRANGE(x) ((x) < LARGEBIN_MINSIZE)
...
# define BIN_INDEX1(x) TER(ISREQ_IN_SMALLRANGE(x), SMALLBIN_INDEX(x),
# define BIN_INDEX2(x) TER(((x) >> 6) <= 32, LARGEBIN1_INDEX(x),
# define BIN_INDEX3(x) TER(((x) >> 9) <= 16, LARGEBIN2_INDEX(x),
# define BIN_INDEX4(x) TER(((x) >> 12) <= 8, LARGEBIN3_INDEX(x),
# define BIN_INDEX5(x) TER(((x) >> 15) <= 4, LARGEBIN4_INDEX(x),
# define BIN_INDEX6(x) LARGEBIN5_INDEX(x)))))
# define SBIN_INDEX(x) BIN_INDEX1(x)
# define BIN_INDEX1_(x) BIN_INDEX2(x) BIN_INDEX3(x)
# define BIN_INDEX2_(x) BIN_INDEX4(x) BIN_INDEX5(x) BIN_INDEX6(x)
# define BIN_INDEX3_(x) SBIN_INDEX1(x) BIN_INDEX1_(x) BIN_INDEX3_(x))
# define LARGEBIN_INDEX(x) BIN_INDEX1_(x) BIN_INDEX2_(x)
# define BIN_INDEX(x) BIN_INDEX3_(x)

typedef struct malloc_arena_s	marena_t;
typedef struct malloc_chunk_s	mchunk_t;
typedef struct pthread_key_t	key_t;
typedef struct pthread_mutex_t	mutex_t;
